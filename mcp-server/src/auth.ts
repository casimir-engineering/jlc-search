...
export async function authMiddleware(c: Context, next: Next): Promise<Response | void> {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return c.json({ error: "Missing Authorization header. Use: Bearer jlc_..." }, 401);
  }

  const token = authHeader.slice(7).trim();
  if (!token.startsWith("jlc_") || token.length !== 36) {
    return c.json({ error: "Invalid API key format" }, 403);
  }
  ...
  if (rows.length === 0) {
    ...
    return c.json({ error: "Invalid or expired API key" }, 403);
  }
